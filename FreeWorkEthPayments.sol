...
contract TaskNetwork is ReentrancyGuard {
    ...
    struct TaskSubmission {
        ...
        uint256 stake; // ETH staked (in wei, e.g., 1 ETH = 10^18 wei)
    ...
    struct Task {
        ...
        uint256 totalStake; // Total ETH staked for submissions
        uint256 rewardPool; // ETH available for rewards
    ...
    struct ValidatorPricing {
        ...
        uint256 baseReward; // Base validator reward (in wei)
    ...
    uint256 public constant BASE_VALIDATOR_REWARD = 1e15; // 0.001 ETH (10^15 wei)
    uint256 public constant MINIMUM_REWARD = 1e15; // 0.001 ETH (10^15 wei)
    ...
    ) public payable taskExists(taskId) nonReentrant {
    ...
    function processValidation(uint256 taskId, uint256 submissionId) internal {
        ...
        if (approveCount > submission.validatorCount / 2) {
            ...
            if (submission.submissionAttempts >= MAX_DAILY_SUBMISSIONS) {
                ...
                submission.stake = 0;
                tasks[taskId].totalStake -= stake;
                payable(submission.user).transfer(stake);
    ...
    function processSubmissionPayout(
        ...
        if (stakeToRefund > 0) {
            payable(submission.user).transfer(stakeToRefund);
        }

        // Pay user reward
        if (userReward > 0) {
            require(tasks[taskId].rewardPool >= userReward, "Insufficient reward pool");
            payable(submission.user).transfer(userReward);
